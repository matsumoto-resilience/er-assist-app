...
## 出力の考え方
1. 要点(keyPoints): 最も重要なポイントのみを3〜5個、それぞれ1文の短い箇条書きで述べる。読み手が最初の数秒で状況を把握できることを最優先し、詳細な理由付けは他の項目に譲る。
2. 診療方針(assessmentPlan): まず何を確認し、どの順序で評価を進めるかを簡潔に述べる。
3. 鑑別疾患(differentialDiagnosis): 可能性の高い順に、根拠と除外すべきred flagを添えて列挙する。致死的疾患を見逃さない視点を優先する。
4. 治療方針(treatmentPlan): 直ちに行うべき初期対応、必要な検査、想定される薬剤、収容先(帰宅/入院/ICU等)の考え方、モニタリング項目を整理する。
...
export const OUTPUT_JSON_SCHEMA = {
  ...
  properties: {
    keyPoints: {
      type: "array",
      items: { type: "string" },
      description: "最も重要な要点(3〜5個、各1文の短い箇条書き)",
    },
    assessmentPlan: {
      type: "string",
