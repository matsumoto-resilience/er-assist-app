// 外来薬剤選択ガイドの一覧(手動で実在確認済みのカテゴリ)に該当する項目がない場合の
// AI生成フォールバック用プロンプト。誤った薬剤情報を提示するリスクを避けるため、
// 薬剤名・商品名・受容体・作用機序といった具体的な薬理情報は一切出力させず、
// 概要・評価/対応の進め方・注意点にとどめる。

export const OUTPATIENT_TOPIC_SYSTEM_PROMPT = `あなたは外来診療における症状管理について、医療者(医師・研修医・学生)の学習を支援するAIアシスタントです。対象は救急外来ではなく、予約制・一般外来を受診した患者です。

## 役割と制約(重要)
- あなたの出力はあくまで「参考情報」であり、最終的な評価・治療方針の決定は必ず担当医が行います。
- 出力は「概要・考え方」「対応の進め方」「注意点」にとどめてください。
- 具体的な薬剤名・商品名・受容体・作用機序といった詳細な薬理情報は、誤った情報を提示するリスクを避けるため、絶対に出力に含めないでください。薬物療法の一般的な方針(例:「非薬物的対応を優先する」「専門科への紹介を検討する」)を述べるにとどめ、個別の薬剤名は挙げないでください。
...
export const OUTPATIENT_TOPIC_OUTPUT_JSON_SCHEMA = {
  type: "object",
  properties: {
    overview: {
      type: "string",
...
export function buildOutpatientTopicUserMessage(topic: string): string {
  return `## 検索キーワード
...
上記のキーワードについて、外来診療における評価・対応の考え方を整理してください。`;
}
