...
import {
  INPATIENT_CATEGORY_GROUP_LABELS,
  INPATIENT_SYMPTOM_CATEGORY_LABELS,
} from "@/lib/inpatient/types";
import { getAllInpatientSymptomGuides } from "@/lib/inpatient/retrieve";
import {
  addSearchHistory,
  clearSearchHistory,
  loadSearchHistory,
} from "@/lib/inpatient/search-history";
import type {
  InpatientAiTopicResult,
  InpatientCategoryGroup,
  InpatientSymptomGuide,
} from "@/lib/inpatient/types";

const guides = getAllInpatientSymptomGuides();
const GROUP_ORDER: InpatientCategoryGroup[] = ["symptom", "chronic"];

export default function InpatientPage() {
  const [query, setQuery] = useState("");
  const [history, setHistory] = useState<string[]>([]);
  const [aiResult, setAiResult] = useState<InpatientAiTopicResult | null>(null);
  ...
  async function handleGenerateTopic() {
    ...
    try {
      const res = await fetch("/api/inpatient/generate-topic", {
        method: "POST",
  ...
  return (
    <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6 lg:px-8">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">入院 薬剤選択ガイド</h1>
        <p className="mt-1 text-sm text-gray-600">
          入院中によく遭遇する症状・慢性疾患の管理について、症状ごとに薬剤選択の考え方・各薬の効果・副作用をまとめた学習用リファレンスです。一覧から選ぶか、症状名で検索してください。
        </p>
      </header>
          ...
          return (
            <div key={group}>
              <h2 className="mb-2 text-sm font-bold text-gray-500">
                {INPATIENT_CATEGORY_GROUP_LABELS[group]}
              </h2>
...
function CategoryListRow({ guide }: { guide: InpatientSymptomGuide }) {
  return (
    <Link
      href={`/inpatient/${guide.category}`}
    ...
    >
      <span className="font-semibold text-gray-900">
        {INPATIENT_SYMPTOM_CATEGORY_LABELS[guide.category]}
      </span>
